...
** Heap
* Asymptotic Notation
- The order of growth of the running time of an algorithm gives a simple way to characterize
  an algorithms efficiency and allows us to compare it with alternative algorithms. Once
  the input becomes large enough, merge sort, with Theta(n lg n) k
** Big O
** Big Theta
** Big Omega
** Common Runtimes
